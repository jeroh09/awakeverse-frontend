...
export default function HeroSection() {
  ...
  const LOOP_MS = 20000; // 17s + 3s pause
  ...
  const runCycle = () => {
    ...
    timeline.forEach((evt) => {
      const id = setTimeout(() => {
        // If collapsed, don’t animate the log; keep header visible only.
        if (isCollapsed && evt.type !== 'HOLD_END') return;
  ...
  useEffect(() => {
    ...
    return () => clearTimers();
    // Re-run the cycle when collapse changes so the UI state is coherent
  }, [prefersReducedMotion, convo, LOOP_MS, userPrompt, timeline, isCollapsed]);
      ...
      <div className="hero-container">
        <div className="hero-scene">
          ...
          <div className="heroChatSlot" aria-label="Conversation preview">
            <div className={`heroChat ${isCollapsed ? 'isCollapsed' : ''}`} role="region" aria-label="Dialogue preview">
              <div className="heroChatHeader">
                <div className="heroChatTitle">
                  ...
                  <button
                    ...
                    aria-label={isCollapsed ? 'Expand dialogue window' : 'Collapse dialogue window'}
              ...
              {/* Body hides when collapsed (desktop/tablet only). Mobile hides chat entirely in CSS. */}
              <div className="heroChatBody">
            ...
            {/* Mobile-only Start Chat overlay */}
            <Link to="/register" className="heroMobileStartChat">
              Start Chat
            </Link>
          </div>
        </div>
