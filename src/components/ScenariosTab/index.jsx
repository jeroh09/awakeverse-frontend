// src/components/ScenariosTab/index.jsx
// ✅ UPDATED: DialoguePill integrated — single-page tab layout
// ✅ All original logic preserved: MarketHub bypass, payment handlers,
//    ThemeToggle, DialogueGuide, UseCaseCarousel, nextScenario switching
...
export default function ScenariosTab({
  ...
  onMarketHubScenarioClosed = null
}) {
  const { user } = useUser();
  ...
  const [currentTheme, setCurrentTheme]                 = useState('awakeverse');
  ...
  // ── Market Hub bypass (original, untouched) ────────────────────────────────
  useEffect(() => {
  ...
  // ── Subscription (original, untouched) ────────────────────────────────────
  ...
  // ── My Scenarios (original, untouched) ────────────────────────────────────
  ...
  const handleScenarioCreated = useCallback((newScenario) => {
    ...
    loadMyScenarios();
    setActiveTab('mine'); // ✅ Show user their new scenario immediately
  }, [loadMyScenarios]);
  ...
  // ✅ Original: supports nextScenario param from InfoPanel scenario switching
  ...
  const handleBlankCreatorClose = () => setShowBlankCreator(false);
  ...
  // ── Payment handlers (original, untouched) ────────────────────────────────
  ...
  // ── Upgrade required — MarketHub bypass preserved ──────────────────────────
  ...
  // ── Chat window — fullscreen takeover (original, untouched) ───────────────
  if (activeScenario) {
    return (
      <ScenarioChatWindow
  ...
  // ── Blank creator — fullscreen takeover (original, untouched) ─────────────
  if (showBlankCreator) {
    return (
      <ScenarioCreator
        template={null}
  ...
  return (
    <div className={`scenarios-tab-container ${currentTheme === 'awakeverse' ? 'theme-awakeverse' : ''}`}>
      ...
      {/* ThemeToggle — original floating position, untouched */}
      <ThemeToggle
        currentTheme={currentTheme}
        onToggle={toggleTheme}
      />

      {/* DialogueGuide modal — original, untouched */}
      <DialogueGuide
        isOpen={showGuide}
      ...
      {/* ✅ Unified pill — sticky, replaces separate section headers */}
      <DialoguePill
        activeTab={activeTab}
...
// ── ScenariosUpgradeRequired (original, untouched) ────────────────────────────
const ScenariosUpgradeRequired = ({ onLearnMore, onUpgradeWithStripe, onUpgradeWithPayPal }) => {
  ...
  const scrollToPricing = () => {
    const el = document.getElementById('dialogue-pricing-section');
    ...
    <div className={`dialogue-upgrade-container ${isDark ? 'theme-dark' : 'theme-light'}`}>
      <div className="dialogue-content-wrapper">
        ...
        <header className="dialogue-page-header">
          <a href="#" className="dialogue-back-button"
        ...
        <section className="dialogue-hero-section">
          <div className="dialogue-hero-content">
            ...
            <button className="dialogue-scroll-to-pricing" onClick={scrollToPricing} aria-label="Scroll to pricing">
          ...
          <div className="dialogue-hero-image-container">
            <img src="/images/upgrade-dialogues-hero.jpg" alt="Dialogue Hub Preview"
              className="dialogue-hero-image"
              onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }}
            />
            <div className="dialogue-hero-image-fallback">Preview: Multi-character debates & scenarios</div>
          </div>
        </section>
        ...
        <section className="dialogue-features-section">
          ...
          <div className="dialogue-features-grid">
            {[
              { icon: '👥', title: 'Multi-Character Debates',  desc: 'Orchestrate conversations between 2-5 AI characters simultaneously.' },
              { icon: '📚', title: 'Curated Templates',        desc: 'Choose from 20+ expertly crafted templates spanning philosophy, ethics, business, science, and politics.' },
              { icon: '🎨', title: 'Custom Scenarios',         desc: 'Build your own dialogue scenarios from scratch. Choose characters, set the context, and define debate objectives.' },
              { icon: '🧠', title: 'Intelligent Moderation',   desc: 'AI-powered facilitation keeps debates on track and maintains philosophical depth.' },
              { icon: '💾', title: 'Continue Anytime',         desc: 'Pause debates anytime and pick up where you left off.' },
              { icon: '🚀', title: 'Priority Access',          desc: 'Get early access to new debate formats, exclusive templates, and experimental multi-AI features.' },
            ].map(({ icon, title, desc }) => (
              <div key={title} className="dialogue-feature-card">
                <span className="dialogue-feature-icon">{icon}</span>
                <h3 className="dialogue-feature-title">{title}</h3>
                <p className="dialogue-feature-description">{desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="dialogue-preview-section">
          ...
          <div className="dialogue-preview-grid">
            {[
              { src: '/images/template-philosophy.jpg', fallback: 'Philosophy Summit', title: 'Philosophy Summit',  desc: 'Socrates, Nietzsche, and Confucius debate the nature of truth and virtue.' },
              { src: '/images/template-business.jpg',   fallback: 'Tech Titans',       title: 'Tech Titans Debate', desc: 'Steve Jobs, Elon Musk, and Bill Gates discuss innovation strategy.' },
              { src: '/images/template-ethics.jpg',     fallback: 'Ethics Council',    title: 'Ethics Council',    desc: 'Kant, Bentham, and Aristotle examine modern moral dilemmas.' },
              { src: '/images/template-science.jpg',    fallback: 'Science Symposium', title: 'Science Symposium', desc: 'Einstein, Hawking, and Curie discuss quantum mechanics and reality.' },
            ].map(({ src, fallback, title, desc }) => (
              <div key={title} className="dialogue-preview-card">
                <div className="dialogue-preview-image-wrapper">
                  <img src={src} alt={title} className="dialogue-preview-image"
                    onError={(e) => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }}
                  />
                  <div className="dialogue-preview-image-fallback">{fallback}</div>
                </div>
                <div className="dialogue-preview-content">
                  <h3 className="dialogue-preview-title">{title}</h3>
                  <p className="dialogue-preview-description">{desc}</p>
        ...
        <section className="dialogue-benefits-section">
          ...
          <div className="dialogue-benefits-grid">
            {[
              'Unlimited Dialogue Scenarios', '20+ Curated Templates',
              'Multi-Character Debates (2-5)', 'Custom Scenario Builder',
              'Save & Resume Conversations', 'AI-Powered Moderation',
              'Priority Feature Access', 'Creator Hub Pro Access',
            ].map(benefit => (
              <div key={benefit} className="dialogue-benefit-item">
                <span className="dialogue-benefit-icon">✓</span>
                <span className="dialogue-benefit-text">{benefit}</span>
              </div>
            ))}
          </div>
        </section>
...
// ── EducationalUpgradeModal (original, untouched) ─────────────────────────────
const EducationalUpgradeModal = ({ isOpen, onClose, onUpgradeWithStripe, onUpgradeWithPayPal }) => {
  ...
  const unlimitedFeatures = [
    { icon: '🎭', title: 'Verse Dialogue Hub',     description: 'Full access to multi-character debates and 20+ Dialogue templates' },
    { icon: '👥', title: 'Multi-AI Conversations', description: 'Create dynamic debates between multiple AI characters' },
    { icon: '📚', title: 'Educational Templates',  description: 'Philosophy, ethics, business, and science debate scenarios' },
    { icon: '💎', title: 'Creator Hub Pro',        description: 'Publish characters and track detailed analytics' },
    { icon: '🚀', title: 'Priority Featuring',     description: 'Your Dialogues get promoted in discovery' },
    { icon: '⚡', title: 'Unlimited Everything',   description: 'No limits on Dialogues, characters, or messages' },
  ];
      ...
      >
        ...
        <div className="dialogue-modal-pricing-card">
          ...
          <div className="dialogue-modal-pricing-features">
            {['Unlimited Dialogues', 'Multi-Character Debates', '20+ Dialogue Templates',
              'Creator Hub Pro Tools', 'All Premium Features', 'VIP Support'].map(f => (
              <div key={f} className="dialogue-modal-pricing-feature">✓ {f}</div>
            ))}
          </div>
