...
// ============================================================================
// STORY MODE — more endpoints (list, archive, context, history)
// ============================================================================

export const getMyStories = async ({ status = 'active', limit = 20, offset = 0 } = {}) => {
  try {
    const response = await api.get(`/stories/my-stories?status=${encodeURIComponent(status)}&limit=${limit}&offset=${offset}`);
    // { stories: [...], count, limit, offset }
...
export const archiveStory = async (storyId) => {
  try {
    const response = await api.post(`/stories/${encodeURIComponent(storyId)}/archive`, {});
    return { status: 'success', ...response.data };
  } catch (error) {
    return {
      status: 'error',
      error: error.response?.data?.error || error.message || 'Failed to archive story',
    };
  }
};

export const getStoryContext = async (storyId) => {
  try {
    const response = await api.get(`/stories/${encodeURIComponent(storyId)}/context`);
    // { story, constraints, indicators: {emotion,momentum,act}, era_label }
    return { status: 'success', ...response.data };
  } catch (error) {
    return {
      status: 'error',
      error: error.response?.data?.error || error.message || 'Failed to load story context',
    };
  }
};

export const getStoryHistory = async (storyId, { limit = 50, before = null } = {}) => {
  try {
    const q = new URLSearchParams();
    if (limit) q.set('limit', String(limit));
    if (before) q.set('before', before);
    const response = await api.get(`/stories/${encodeURIComponent(storyId)}/history?${q.toString()}`);
    // { messages: [{role:'user'|'assistant', content, ts}], has_more }
    return { status: 'success', ...response.data };
  } catch (error) {
    return {
      status: 'error',
...
// ============================================================================
// STREAMING send (uses fetch for ReadableStream) — cookies + CSRF
// ============================================================================
const getCsrf = () => {
  const m = document.cookie.match(/(?:^|;\s*)av_csrf=([^;]+)/);
...
export const streamStoryMessage = async ({ storyId, text, onToken, onDone, onError, signal }) => {
  // Endpoint expected to return text/event-stream or chunked text
  const url = `${api.defaults.baseURL}/stories/${encodeURIComponent(storyId)}/message`;
  try {
    ...
    // Handle either SSE (split by "data:") or raw chunks
    ...
    while (true) {
      ...
      for (let i = 0; i < parts.length - 1; i++) {
        ...
        if (m && onToken) onToken(m[1]);
      }
      buffer = parts[parts.length - 1];
    }

    // Flush any remaining non-SSE content as a single token
    if (buffer && onToken) onToken(buffer);
